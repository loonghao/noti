...
use common::{
    ...
    spawn_server_sqlite_with_workers,
...
common::dual_backend_test!(
    with_workers,
    e2e_purge_removes_terminal_preserves_nonterminal,
    e2e_sqlite_purge_removes_terminal_tasks,
    |spawn_fn, label| {
        let (base, worker_handle) = spawn_fn().await;
        let client = test_client();
        ...
        assert!(total_before >= 3, "{label}should have at least 3 tasks");
        ...
        assert!(
            purged >= 2,
            "{label}should purge at least the completed + failed tasks"
        );
...
common::dual_backend_test!(
    without_workers,
    e2e_batch_async_mixed_providers_and_priorities,
    e2e_sqlite_batch_async_mixed_providers_and_priorities,
    |spawn_fn, label| {
        let (callback_base, payloads) = spawn_callback_server().await;

        let (base, state) = spawn_fn(vec![Arc::new(MockOkProvider)]).await;
                ...
                "items": [
                    {
                        ...
                        "text": "{label}invalid-high",
                        "priority": "high",
                        "metadata": {"order": "invalid-high"}
                    },
                    {
                        "provider": "mock-ok",
                        "text": "{label}valid-low",
                        "priority": "low",
                        "callback_url": &callback_url,
                    ...
                    },
                    {
                        ...
                        "text": "{label}invalid-low",
                        "priority": "low",
                        "metadata": {"order": "invalid-low"}
                    },
                    {
                        "provider": "mock-ok",
                        "text": "{label}valid-normal",
                        "priority": "normal",
                        "callback_url": &callback_url,
        ...
        let results = body["results"].as_array().unwrap();
        assert!(!results[0]["success"].as_bool().unwrap()); // index 0: invalid
        assert!(results[1]["success"].as_bool().unwrap());   // index 1: valid
        assert!(results[2]["success"].as_bool().unwrap());   // index 2: valid
        assert!(!results[3]["success"].as_bool().unwrap()); // index 3: invalid
        assert!(results[4]["success"].as_bool().unwrap());   // index 4: valid
        ...
        // Verify callback order: urgent → normal → low (only valid items)
        {
            ...
            assert!(
                received.len() >= 3,
                "{label}expected at least 3 callbacks, got {}",
                received.len()
            );
...
common::dual_backend_test!(
    with_workers_serial,
    e2e_batch_async_mock_fail_provider_with_priorities,
    e2e_sqlite_batch_async_mock_fail_provider_with_priorities,
    |spawn_fn, label| {
        let (callback_base, payloads) = spawn_callback_server().await;

        // serial helper registers MockOkProvider + MockFailProvider
        let (base, worker_handle) = spawn_fn(vec![]).await;

        let client = test_client();
        let callback_url = format!("{callback_base}/callback");

        // InMemory: 4 items with full priority spread; SQLite: 3 items focused on urgent/low ordering
        let resp = client
            .post(format!("{base}/api/v1/send/async/batch"))
            .json(&json!({
                "items": [
        ...
        assert_eq!(body["enqueued"], total, "{label}all items use registered providers");
        assert_eq!(body["failed"], 0, "{label}no enqueue-time failures");
        ...
        let task_ids: Vec<String> = results
            ...
            .map(|r| {
                assert!(
                    ...
                    "{label}all items should succeed at enqueue"
                );
                r["task_id"].as_str().unwrap().to_string()
            })
            .collect();
        assert_eq!(task_ids.len(), total as usize);
        ...
        for (order, status) in &statuses {
            if order.starts_with("fail-") {
                assert_eq!(
                    status, "failed",
                    "{label}{order} should have failed status"
                );
            } else {
                assert_eq!(
                    status, "completed",
                    "{label}{order} should have completed status"
                );
            }
        }
        ...
        // Verify callback priority ordering
        {
            let received = payloads.lock().unwrap();
            assert!(
                received.len() >= total as usize,
                "{label}expected at least {total} callbacks, got {}",
                received.len()
            );

            // Urgent items should come first; last item should be lowest priority
            let first_order = received[0]["metadata"]["order"].as_str().unwrap();
            assert!(
                first_order.contains("urgent"),
                "{label}first callback should be urgent, got '{first_order}'"
            );

            let last_idx = received.len() - 1;
            let last_order = received[last_idx]["metadata"]["order"].as_str().unwrap();
            assert!(
                last_order.contains("low") || last_order.contains("normal"),
                "{label}last callback should be low/normal priority, got '{last_order}'"
            );
        }
...
common::dual_backend_test!(
    with_workers_serial,
    e2e_concurrent_batch_async_requests_all_accepted,
    e2e_sqlite_concurrent_batch_async_requests,
    |spawn_fn, label| {
        let (callback_base, payloads) = spawn_callback_server().await;
        ...
        // InMemory: 5 batches × 2 items; SQLite: 4 batches × 3 items (with mock-fail)
        let num_batches: u32 = if label.is_empty() { 5 } else { 4 };
        let has_fail_item = !label.is_empty();
        ...
        for batch_idx in 0..num_batches {
            ...
            handles.push(tokio::spawn(async move {
                let mut items = vec![
                    json!({
                        "provider": "mock-ok",
                        "text": "{label}b{batch_idx}-0",
                        "priority": if label.is_empty() { "normal" } else { "high" },
                        "callback_url": &cb_url,
                        "metadata": {"batch": format!("{batch_idx}"), "item": "0"}
                    }),
                    json!({
                        "provider": "mock-ok",
                        "text": "{label}b{batch_idx}-1",
                        "priority": "normal",
                        "callback_url": &cb_url,
        ...
        for handle in handles {
            ...
            assert_eq!(
                status,
                StatusCode::ACCEPTED,
                "{label}all batch requests should return 202"
            );

            let total = body["total"].as_u64().unwrap();
            assert_eq!(body["enqueued"], total, "{label}all providers registered");
            assert_eq!(body["failed"], 0);
        ...
        let expected_total: usize = if label.is_empty() { 10 } else { 12 };
        assert_eq!(all_task_ids.len(), expected_total, "{label}task count mismatch");
        ...
        if label.is_empty() {
            assert_eq!(completed, expected_total as u32, "{label}expected all completed");
        } else {
            assert_eq!(completed, 8, "{label}expected 8 completed");
            assert_eq!(failed, 4, "{label}expected 4 failed");
        }
...

        {
            ...
            assert_eq!(
                received.len(),
                expected_total,
                "{label}expected {expected_total} callbacks, got {}",
                received.len()
            );
        }
...
common::dual_backend_test!(
    with_workers_serial,
    e2e_batch_async_mixed_retry_policies,
    e2e_sqlite_batch_async_mixed_retry_policies,
    |spawn_fn, label| {
        let (callback_base, payloads) = spawn_callback_server().await;
        let flaky: Arc<dyn noti_core::NotifyProvider> = Arc::new(MockFlakyProvider::new(2));
        let (base, worker_handle) = spawn_fn(vec![flaky]).await;
        ...
        let resp = client
            ...
            .json(&json!({
                "items": [
                    {
                        "provider": "mock-flaky",
                        "text": "{label}flaky with enough retries",
                        "retry": {"max_retries": 5, "delay_ms": 10},
                        "callback_url": &callback_url
                    },
                    {
                        "provider": "mock-fail",
                        "text": "{label}always fails with zero retries",
                        "retry": {"max_retries": 0, "delay_ms": 10},
                        "callback_url": &callback_url
                    },
                    {
                        "provider": "mock-ok",
                        "text": "{label}always succeeds",
                        "callback_url": &callback_url
                    },
                    {
                        "provider": "mock-fail",
                        "text": "{label}always fails with retries",
                        "retry": {"max_retries": 2, "delay_ms": 10},
                        "callback_url": &callback_url
                    }
                ]
        ...
        assert_eq!(
            ...
            "{label}flaky with enough retries should complete"
        );

        assert_eq!(
            terminal_statuses[1]["status"], "failed",
            "{label}mock-fail with zero retries should fail"
        );
        assert_eq!(
            terminal_statuses[1]["attempts"].as_u64().unwrap(),
            1,
            "{label}zero-retry mock-fail should only try once"
        );
        ...
        assert_eq!(
            ...
            "{label}mock-fail with retries should still fail"
        );
        assert!(
            terminal_statuses[3]["attempts"].as_u64().unwrap() >= 3,
            "{label}mock-fail should exhaust all retries"
        );
        ...
        tokio::time::sleep(Duration::from_millis(300)).await;
        {
            ...
            assert_eq!(
                received.len(),
                4,
                "{label}expected 4 callbacks, got {}",
                received.len()
            );
        }
