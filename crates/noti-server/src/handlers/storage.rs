...
pub async fn store_file(
    ...
    let ext = std::path::Path::new(original_name)
        ...
        .and_then(|e| e.to_str())
        .map(|e| {
            // Keep only alphanumeric characters from the extension
            let sanitized: String = e.chars().filter(|c| c.is_alphanumeric()).collect();
...
/// Validate that a file_id is a proper UUID to prevent path traversal attacks.
/// File IDs are generated by the server as UUIDs, so any non-UUID input is
/// either malformed or an attempt to inject path components like `../`.
fn validate_file_id(file_id: &str) -> Result<(), ApiError> {
    if file_id.parse::<Uuid>().is_err() {
        return Err(ApiError::bad_request("invalid file_id: must be a valid UUID"));
    }
    Ok(())
}
