...
- [x] ~~`crates/noti-server/tests/e2e_test.rs` — the file is still monolithic and mixes health/auth/CORS/body-limit/queue/retry/scheduling scenarios; split by concern while keeping shared helpers in `tests/common/mod.rs`~~ — split into 20 focused files (e2e_health_meta, e2e_providers, e2e_send, e2e_openapi, e2e_status, e2e_middleware, e2e_worker_callback, e2e_priority, e2e_per_ip_rate, e2e_sqlite_priority, e2e_batch_priority, e2e_stale_recovery, e2e_queue_purge, e2e_templates, e2e_concurrent, e2e_errors, e2e_batch, e2e_retry, e2e_backoff, e2e_scheduled); `dual_backend_test!` macro now uses `#[macro_export]`
- [x] ~~`crates/noti-providers/tests/provider_send_test.rs` + `provider_send_extended_test.rs` — many provider send-path tests repeat the same success/failure/metadata contract shape; consider a shared contract-test macro/DSL and family-based file splits~~ — deferred: both test files compile clean; provider tests are sufficiently independent; the contract shape is enforced by `NotifyProvider` trait, not duplication
- [x] ~~`crates/noti-server/src/handlers/queue.rs` — request/response DTOs, schedule parsing, queue error mapping, HTTP handlers, and unit tests still live in one module; extract `dto`/`service`/`handlers` seams when safe~~ — split into `queue/dto.rs` (request/response types + QueueStats impl), `queue/service.rs` (task_to_info, parse_scheduled_time, parse_task_status, queue_error), `queue/handlers.rs` (HTTP handler functions), `queue/mod.rs` (re-exports + unit tests); openapi.rs updated to reference `queue::handlers`
...
- [x] ~~`crates/noti-providers/src/apns.rs` — `p256 = { version = "0.13", features = ["ecdsa"] }` is pinned directly in the crate `Cargo.toml` rather than as a workspace dependency~~ — promoted to `[workspace.dependencies]` in `Cargo.toml`; `noti-providers/Cargo.toml` now uses `p256 = { workspace = true }` (62b019e)
- [x] ~~`crates/noti-server/src/handlers/prometheus.rs` — Prometheus metrics handler builds strings manually; consider replacing with the `metrics` crate to reduce maintenance burden~~ — migrated to `prometheus-client` crate with proper `Registry`, `Family<Label, Gauge>`, `encode()` — structured, maintainable, and produces correct Prometheus exposition format (706b5e9)
- [x] ~~`crates/noti-core/src/circuit_breaker.rs` — 3 timing-sensitive tests are marked `#[ignore]` (half-open transition, close-on-success-in-half-open, reopen-on-failure-in-half-open); these cannot reliably use `thread::sleep` in unit tests; consider replacing with a `MockClock` abstraction or moving to integration tests with tokio's `time::pause`/`time::advance`~~ — resolved: iteration agent added `Clock` trait + `MockClock` (Arc-based shared state) + `CircuitBreaker<C>` generic; all 3 tests now run deterministically (1fdd434)
