...
- [ ] `crates/noti-server/src/handlers/storage.rs` — `download_file` and `delete_file` both contain identical "scan upload dir, find file starting with `{file_id}.`" logic (~15 lines each); extract a `find_file_by_id(dir, file_id) -> Option<PathBuf>` helper to eliminate duplication
