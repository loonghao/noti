...
- [ ] `crates/noti-providers/src/twilio.rs:135` — MMS attachment fallback path `attachment.read_bytes().await.unwrap_or_default()` silently sends a `data:;base64,` empty data URI when read fails; should propagate the error or skip the attachment and log a warning instead
- [ ] `crates/noti-providers/tests/provider_send_test.rs` — file has grown to 2672 lines with 33 provider modules; providers without `base_url` override (wecom, discord, telegram, feishu, etc.) only test `validate_config` and metadata — consider splitting into `provider_validation_test.rs` and `provider_send_wiremock_test.rs` in a future round when the file exceeds 3000 lines
