...
- [ ] `crates/noti-providers/src/lib.rs` — `register_all_providers()` is a long manual registry list; consider a list-driven or macro-driven registration table to reduce drift risk as providers keep growing (now 126+ providers) — **NOTE 2026-04-08**: iteration agent attempted a `paste!`-macro-based refactor in the workspace but left an incomplete WIP with compile errors (`decl_providers_mixed!` helper arms used `[<...>]` paste syntax outside a `paste!` block, `decl_providers_no_client` macro unused). WIP was discarded; tracking here for a correct implementation in a future round.
- [ ] `crates/noti-queue/benches/queue_throughput.rs` — benchmark runtime is now created outside `b.iter()` via `make_runtime()` helper; consider using `criterion::async_executor` or `b.to_async()` for cleaner async benchmark integration
- [x] ~~P2 Performance: Rate limiter global Mutex bottleneck (consider `dashmap` or sharded approach)~~ — replaced `ip_buckets` with DashMap for per-IP sharded locking; switched `global_bucket` from `tokio::sync::Mutex` to `std::sync::Mutex` (51085c4)
- [x] ~~`crates/noti-providers/src/twilio.rs:137-142` — MMS attachment fallback path unconditionally constructs `data:` URI when media upload fails~~ — fixed: replaced with warning log and skip (b30991a)
- [x] ~~`crates/noti-queue/src/sqlite.rs` — `move_to_dlq` manually constructs `TaskRow` via positional `row.get(N)` instead of using `TaskRow::from_rusqlite_row`~~ — fixed: now uses `SELECT *` + `TaskRow::from_rusqlite_row` (733c50d)
- [x] ~~`crates/noti-providers/tests/provider_send_test.rs` — file had grown to 21,682 lines with 140 provider modules~~ — split into 10 category-specific files under `provider_send/` subdirectory: chat, push, sms, email, webhook, automation, home, media, misc, rate_limit (commit TBD)
- [x] ~~`crates/noti-server/src/handlers/storage.rs` — `download_file` and `delete_file` both contain identical "scan upload dir, find file starting with `{file_id}.`" logic (~15 lines each); extract a `find_file_by_id(dir, file_id) -> Option<PathBuf>` helper to eliminate duplication~~ — extracted `find_file_by_id()` helper (d9861f5)
